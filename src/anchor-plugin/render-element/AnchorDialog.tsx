...
export function AnchorDialog({
  ...
  /**
   * When present, the dialog represents an internal embed (`![[spec]]`)
   * instead of an anchor. `target` is the link target (incl. any `#heading`);
   * edit/remove are delegated to the host (image element transforms).
   */
  embed?: {
    target: string
    onEdit: () => void
